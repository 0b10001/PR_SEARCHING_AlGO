...
function generateRandomArray() {
    ...
    while (boxesE > 0) {
        for (let i = 0; i < (boxesE); i++) {
            ...
            emptyBox.id = emptyBoxId.toString();
            emptyBoxId = emptyBoxId + 1;
...
function checkArrays() {
    ...
    for(let i = 0; i < emptyBoxes.length - 1; i++) {
      var stuff = document.getElementById(i.toString()); // reset the color of the empty boxes
      stuff.style.backgroundColor = "#f0f0f0";
    }
    ...
    if (differences === true) {
      ...
      var stuff = document.getElementById(differences[0].toString()); 
      stuff.style.backgroundColor = "red"; // highlight the box with the first error found
